...
public class HzMapService {
    ...
    public MapStats getMapStats(Member member, String mapName) {
        ...
        Optional<Boolean> showConfig = ThreadStorage.getOptional("showConfig");
        Optional<Boolean> showKeys = ThreadStorage.getOptional("showKeys");
        Optional<Boolean> showValues = ThreadStorage.getOptional("showValues");
        Optional<Boolean> showMap = ThreadStorage.getOptional("showMap");

        ObjectStatsCollector<MapStats> task = new ObjectStatsCollector<>(MapStats.class, map.getName(), showConfig.orElse(false), showKeys.orElse(false), showValues.orElse(false), showMap.orElse(false));
        ...
        try {
            ...
            log.error("Get MapStats operation for Map={}, Member={} failed.", map.getName(), member.getAttribute("hostname"));
            e.printStackTrace();
        }
