...
public class HazelcastStatsCollector {
    ...
    public HazelcastStats getHazelcastStats(QueryType queryType) {
        Set<Member> members = queryType == QueryType.LOCAL ? Set.of(HazelcastUtil.getInstance().getCluster().getLocalMember()) : HazelcastUtil.getInstance().getCluster().getMembers();
    ...
    public List<DistributedObjectDto> getDistributedObjectInfoList(QueryType queryType) {
        Set<Member> members = queryType == QueryType.LOCAL ? Set.of(HazelcastUtil.getInstance().getCluster().getLocalMember()) : HazelcastUtil.getInstance().getCluster().getMembers();
        return queryType == QueryType.CLUSTER ? List.of(getDistributedObjectInfoCluster(members)) : members.stream().map(this::getDistributedObjectInfoList).toList();
    }
    ...
    private DistributedObjectDto getDistributedObjectInfoList(Member member) {
        ...
        for (DistributedObject object : distributedObjects) {
            DistributedObjectStats distributedObjectStats = getDistributedObjectStats(member, object);
    ...
    private DistributedObjectStats getDistributedObjectStats(Member member, DistributedObject object) {
        if (IMap.class.isAssignableFrom(object.getClass())) {
            return hzMapService.getMapStats(member, object.getName(), false);
