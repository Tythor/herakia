...
public class JacksonByteArraySerializer<T> implements ByteArraySerializer<T> {
    ...
    public T read(byte[] buffer) throws IOException {
        ...
        if (type != null) {
            return objectMapper.readValue(Zstd.decompress(buffer, (int) Zstd.getFrameContentSize(buffer)), type);
        } else {
            return objectMapper.readValue(Zstd.decompress(buffer, (int) Zstd.getFrameContentSize(buffer)), new TypeReference<>() {});
        }
    }
    ...
    protected byte[] logWrite(Object object) throws IOException {
    ...
    protected T logRead(byte[] buffer) throws IOException {
        long startTime = System.nanoTime();
