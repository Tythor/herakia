...
public abstract class AbstractLockBuilder {
    ...
    public class LockTypeStep {
        ...
        @SneakyThrows
        public SleepOrTaskStep tryLock(boolean throwUnchecked) {
            AbstractLockBuilder.this.throwUnchecked = throwUnchecked;
            return tryLock();
        }

        public SleepOrTaskStep tryLock(boolean throwUnchecked, Duration waitTime) {
            AbstractLockBuilder.this.waitDuration = waitTime;
            return tryLock(throwUnchecked);
        }

        public SleepOrTaskStep tryLock(boolean throwUnchecked, Duration waitTime, Duration leaseTime) {
            AbstractLockBuilder.this.leaseDuration = leaseTime;
            return tryLock(throwUnchecked, waitTime);
        }
    }
