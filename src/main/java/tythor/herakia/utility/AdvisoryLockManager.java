...
public class AdvisoryLockManager {
    ...
    public AdvisoryLockManager(DataSource dataSource) {
        this.dataSource = createHikariDataSource(dataSource);
    }

    private HikariDataSource createHikariDataSource(DataSource dataSource) {
        HikariConfig hikariConfig = new HikariConfig();
        ((HikariDataSource) dataSource).copyStateTo(hikariConfig);
    ...
    public AdvisoryLock obtain(Object key) {
        return lockMap.computeIfAbsent(key, k -> new AdvisoryLock(k, connectionMap, dataSource));
    }
...
//            if (!delegate.tryLock(100, TimeUnit.MICROSECONDS)) return false;
        ...
        public boolean isLocked() {
            ...
            String query = "SELECT COUNT(*) FROM pg_locks " +
                "WHERE database = (SELECT oid FROM pg_database WHERE datname = current_database()) " +
                "AND locktype = 'advisory' " +
                "AND objid = ?";

            SqlRowSet sqlRowSet = jdbcTemplate.queryForRowSet(query, key.hashCode());
        ...
        public boolean forceUnlock() {
            ...
            String query = "SELECT pg_terminate_backend(pid) FROM pg_locks " +
                "WHERE database = (SELECT oid FROM pg_database WHERE datname = current_database()) " +
                "AND locktype = 'advisory' " +
                "AND objid = ?";
