 ...
 * <p>
 * <b>Note:</b> {@link org.springframework.cache.Cache#getNativeCache()} must be an implementation of {@link com.hazelcast.map.IMap}.
 */
@Slf4j
@Component
    ...
    public void init() {
        ...
        for (Method method : cacheRefreshMethods) {
            ...
            if (profiles.length > 0 && environment.matchesProfiles(profiles)) {
                taskScheduler.schedule(() -> executeScheduledTask(method, cacheRefresh), new CronTrigger(cacheRefresh.cron()));
            }
        }
    }

    private void executeScheduledTask(Method method, CacheRefresh cacheRefresh) {
        IMap<Object, Object> cacheRefreshMap = HazelcastUtil.getMap("cacheRefresh");
        try {
